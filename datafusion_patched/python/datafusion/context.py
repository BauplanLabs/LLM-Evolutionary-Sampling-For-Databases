...
class SessionContext:
    ...
    @classmethod
    def new_without_join_selection(
        cls,
        config: SessionConfig | None = None,
        runtime: RuntimeEnvBuilder | None = None,
    ) -> SessionContext:
        """Create a new :py:class:`SessionContext` with basic join selection only.
        
        This excludes statistics-based join optimization while keeping the basic
        join selection functionality needed to make plans executable. This is useful
        for testing or scenarios where you want to disable automatic join optimization
        ...
        Returns:
            A new :py:class:`SessionContext` with basic join selection only.
        """
        config_internal = config.config_internal if config else None
        runtime_internal = runtime.config_internal if runtime else None
        
        internal_ctx = SessionContextInternal.new_without_join_selection(
            config_internal, runtime_internal
        )
    ...
    def collect(self, plan: ExecutionPlan) -> list[pa.RecordBatch]:
        """Collect the results of the ``plan``."""
        return self.ctx.collect(plan._raw_plan)
